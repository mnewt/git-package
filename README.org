...
**** =:dir=
     The local directory where the files should be cloned. It is relative to the custom variable =git-package-user-directory=.
...
*** =git-package-clean-unused=
    Delete any package directories in =git-package-user-directory= which have not been loaded using =git-package=.
